...
public class VoteCommand implements CommandExecutor {
    ...
    public VoteCommand(UltimateUtilities ultimateUtilities){
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if(ultimateUtilities.getPluginConfig().getBoolean(ConfigPaths.VOTE_COMMAND.getEnablePath())){
            if(!(sender instanceof Player)){
                for(String str : new MessageUtils(ultimateUtilities).getMessages(sender, ConfigPaths.VOTE_COMMAND_MESSAGE)){
                    sender.sendMessage(str);
                }
            }else{
                Player player = (Player) sender;
                for(String str : new MessageUtils(ultimateUtilities).getMessages(player, ConfigPaths.VOTE_COMMAND_MESSAGE)){
