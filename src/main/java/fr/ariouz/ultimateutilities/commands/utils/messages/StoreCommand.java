...
public class StoreCommand implements CommandExecutor {
    ...
    public StoreCommand(UltimateUtilities ultimateUtilities){
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if(ultimateUtilities.getPluginConfig().getBoolean(ConfigPaths.STORE_COMMAND.getEnablePath())){
            if(!(sender instanceof Player)){
                for(String str : new MessageUtils(ultimateUtilities).getMessages(sender, ConfigPaths.STORE_COMMAND_MESSAGE)){
                    sender.sendMessage(str);
                }
            }else{
                Player player = (Player) sender;
                for(String str : new MessageUtils(ultimateUtilities).getMessages(player, ConfigPaths.STORE_COMMAND_MESSAGE)){
